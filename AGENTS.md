...
Executable tooling is governed by the checked-in runtime/configuration (`package.json`, `pnpm-lock.yaml`, test configuration, and `.github/workflows/ci.yml`). Durable product behavior and architecture remain governed by the relevant docs/decisions. Agent authority and merge evidence are governed by `docs/agent-permission-model.md` and `docs/human-light-merge-gate.md`.
...
Before normal Phase 1 product implementation proceeds, complete the remaining repository-safety setup for Guardrail Integrity and `main` Ruleset protection.
...
- `docs/human-light-merge-gate.md`
...
- Follow `docs/agent-permission-model.md`; ordinary coding/review agents do not own merge, repository administration, Guardrail approval, or production authority.
- Classify non-trivial changes using `docs/human-light-merge-gate.md` and produce the tier-appropriate evidence.
...
Phase 0 established the application/runtime and verification foundation. The immediate repository-safety follow-up is to complete Guardrail Integrity and then require the repository verification statuses on `main` through a GitHub Ruleset before normal product implementation proceeds.
